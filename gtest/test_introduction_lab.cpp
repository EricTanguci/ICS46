...
TEST(TokenTests, IsMaxTrueOnlyAtMaxValue) {
    Token tMax{"None", Token::MAX_VALUE};
    Token tNotMax{"None", Token::MAX_VALUE - 1};
    EXPECT_TRUE(tMax.isMax());
    EXPECT_FALSE(tNotMax.isMax());
}
...
TEST(BagTests, AddTokensReturnsSumOfAllTokenValues) {
    static const int NUM_TOKENS = 16;
    Bag bag(NUM_TOKENS);
...
TEST(BagTests, HighValueTokensReturnsCountOfHighValueTokens) {
    static const int NUM_TOKENS = 16;
    Bag bag(NUM_TOKENS);
    BagSetupResult expected = setupBagWithKnownContents(bag);
...
TEST(BagTests, FirstGreenReturnsIndexOfFirstGreenOrMinusOne) {
    static const int NUM_TOKENS = 16;
    Bag bag(NUM_TOKENS);
    BagSetupResult expected = setupBagWithKnownContents(bag);
